...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <div class="buttons">
                <a href="{{ route('student-data.export') }}"
                    class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm btn-success me-1"><i
                        class="fas fa-file-export fa-sm text-white"></i> Export Data</a>
                <a href="{{ route('violation-data.export') }}"
                    class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm btn-warning me-1"><i
                        class="fas fa-file-export fa-sm text-white"></i> Export Data Pelanggaran</a>
        ...
        <div class="table-responsive">
            <form action="{{ route('student-data.delete') }}" method="post">
                @csrf
                        ...
                        @if (count($students) > 0)
                            @foreach ($students as $item)
                                <tr onclick="toggleCheckbox(event, '{{ $item->id }}')">
                                    ...
                                    <td>
                                        <a href="{{ route('student-data.detail', $item->uuid) }}"
                                            class="custom-link action-col">
                                            {{ $item->name }}
                                        </a>
                                    </td>
                                    <td>{{ Carbon\Carbon::parse($item->born_date)->locale('id')->isoFormat('D MMMM Y') }}
                                    </td>
                                    ...
                                    <td class="text-nowrap">
                                        <div class="d-flex flex-column">
                                            <div class="align-items-center d-grip gap-4 mx-auto">
                                                {{-- Edit Button --}}
                                                <a href="{{ route('student-data.edit', $item->uuid) }}"><button
                                                        type="button" class="btn btn-sm btn-warning action-col">
                                                ...
                                                {{-- Delete Button --}}
                                                <button type="button" class="btn btn-danger btn-sm action-col"
                                                    data-toggle="modal" data-target="#deleteStudentDataModal">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="1em"
                                                        viewBox="0 0 448 512">
                                                        <style>
                                                            svg {
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title fw-bold text-danger" id="student-data-modal-label">
                                    Konfirmasi Hapus</h5>
